...
pub(crate) struct KiraSchedulerConfig {
...
impl Default for KiraSchedulerConfig {
    fn default() -> Self {
        Self {
...
pub struct KiraScheduler<Tx: TransactionWithMeta> {
    common: SchedulingCommon<Tx>,
    working_account_set: ReadWriteAccountSet,
    unschedulables: Vec<TransactionPriorityId>,
    config: KiraSchedulerConfig,
}

impl<Tx: TransactionWithMeta> KiraScheduler<Tx> {
    #[cfg_attr(feature = "dev-context-only-utils", qualifiers(pub))]
    pub(crate) fn new(
...
impl<Tx: TransactionWithMeta> Scheduler<Tx> for KiraScheduler<Tx> {
...
mod test {
    ...
    fn create_test_frame(
        ...
        KiraScheduler<RuntimeTransaction<SanitizedTransaction>>,
        ...
        let scheduler =
            KiraScheduler::new(consume_work_senders, finished_consume_work_receiver, config);
        (
            scheduler,
    ...
    fn test_schedule_disconnected_channel() {
        let (mut scheduler, work_receivers, _finished_work_sender) =
            create_test_frame(1, KiraSchedulerConfig::default());
    ...
    fn test_schedule_simple_thread_selection() {
        let (mut scheduler, work_receivers, _finished_work_sender) =
            create_test_frame(2, KiraSchedulerConfig::default());
        let mut container =
